...
class BankAccount:
    ...
    def open_account(self, kwargs):
        ...
        self.aadhar_no = kwargs["aadhar_no"] if "aadhar_no" in kwargs else None
        ...
        with open("data.json", "r", encoding="utf-8") as file:
            ...
            if account_no is not None or atm_card_no is not None or loan_account_no is not None:
                for customer in data:
                    if account_no == customer["account_no"] or atm_card_no == customer["atm"][
                        "card_no"] or loan_account_no == customer["loan_account"]["loan_account_no"]:
...
"""
    Single Inheritance : 1 Base class ---> 1 Derive class  (BankAccount ---> AtmCard)
    Multiple Inheritance : 1 base class ---> Multiple derive class  (BankAccount ---> AtmCard & LoanAccount)
    Multilevel Inheritance : 1 base class ---> Derive class ---> Derive class
"""
...
while True:
    ...
    if q == "Q":
        ...
        choose = int(input(""""
        1 for open account
        2 for deposit in account
        3 for withdrew from account
        4 for check balance
        ...
        if choose == 1:
            ...
            if len(adhar_card) == 12 and adhar_card.isdigit() == True:
                user_input["aadhar_no"] = adhar_card
            else:
                print("please inter the coreect no")

            if pancard_no[:2].isalpha() == True and pancard_no[3] in ["A", "B", "C", "F", "G", "H", "L", "J", "P",
                                                                      "T"] and pancard_no[5:-1].isdigit() == True and \
                    pancard_no[-1].isalpha() == True:
